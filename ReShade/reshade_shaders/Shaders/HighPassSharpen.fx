...
uniform float HighPassSharpOffset < __UNIFORM_SLIDER_FLOAT1
	...
	ui_label = "偏移量";
	ui_tooltip = "模糊半径的额外调整。小于1.00的值会减小半径，将锐化限制在更精细的细节上。";
> = 1.00;

uniform int HighPassBlendMode <
	ui_type = "combo";
	ui_label = "混合模式";
	ui_items = "柔光\0叠加\0正片叠底\0强光\0亮光\0滤色\0线性光\0相加";
	ui_tooltip = "混合模式决定如何将锐化遮罩应用于原始图像";
> = 1;

uniform int HighPassBlendIfDark < __UNIFORM_SLIDER_INT1
	ui_min = 0; ui_max = 255;
	ui_label = "暗部排除";
	ui_tooltip = "任何低于此值的像素都将被排除在效果之外。设置为50以针对中间调。";
> = 0;

uniform int HighPassBlendIfLight < __UNIFORM_SLIDER_INT1
	ui_min = 0; ui_max = 255;
	ui_label = "亮部排除";
	ui_tooltip = "任何高于此值的像素都将被排除在效果之外。设置为205以针对中间调。";
> = 255;

uniform bool HighPassViewBlendIfMask <
	ui_label = "显示混合遮罩";
	ui_tooltip = "显示混合遮罩。效果不会应用于黑色覆盖的区域。";
> = false;
