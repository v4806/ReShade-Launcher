...
float4 GetRotator(float angle)
{
    ...
    return float4(sc.yx, -sc.x, sc.y);
}

float2 Rotate2D(float2 vect, float4 rot)
{
    return float2(dot(vect, rot.xy), dot(vect, rot.zw));
}
...
{
    uint feature_mip = max(0, mip - MIN_MIP); // +1 for better results
    float2 texelsize = BUFFER_PIXEL_SIZE * exp2(mip + 1); // independent of mot_samp size
    float2 scale = texelsize * (mip > 0 ? UI_MV_SCALE : 1.5);
    float qrand = GetR1(GetBlueNoise(i.vpos.xy).x, mip + 1);
    float4 rotator = GetRotator(qrand * HALF_PI); // max 90deg rotation
    ...
    {
        float2 sample_uv = i.uv + Rotate2D(float2(x, y), rotator) * scale;
        ...
        float wm = dot(sample_gbuf.xy, sample_gbuf.xy) * 250.0; // long motion
        float weight = exp2(-(wz + wm) * 4.0);
