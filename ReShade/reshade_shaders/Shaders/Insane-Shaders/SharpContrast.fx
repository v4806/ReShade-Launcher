...
namespace SharpContrast
{
	...
	uniform bool GammaCorrect<
		ui_label = "使用线性色域";
		ui_tooltip = "更改锐化是应用于线性色域还是标准色域。";
	> = true;

	uniform bool EnableDepth<
		ui_category = "深度设置";
		ui_label = "启用深度设置";
		ui_tooltip = "必须启用此设置才能调整深度曲线或启用遮罩天空选项。";
	> = false;
	...
	uniform float DepthCurve<
		...
		ui_label = "深度曲线";
		ui_category = "深度设置";
		ui_tooltip = "调整锐化随深度衰减的方式，较低的值意味着更短的衰减距离。";
		ui_min = 0; ui_max = 1;
		ui_step = 0.001;
	> = 1;
	...
	uniform bool MaskSky<
		ui_category = "深度设置";
		ui_label = "遮罩天空";
		ui_tooltip = "此设置可防止天空应用锐化。";
	> = true;


	uniform int Debug<
		ui_type = "combo";
		ui_items = "无\0遮罩\0锐化\0";
		ui_label = "调试视图";
		ui_category = "调试";
		ui_tooltip = "遮罩：显示锐化应用受限的区域。\n"
					 "锐化：显示应用于图像的锐化。";
	> = 0;
	...
	#endif
	technique SharpContrast< ui_tooltip =     "SharpContrast是一种锐化着色器，它考虑了比大多数锐化着色器更大的屏幕区域，\n"
						  "同时也试图避免跨越对比度边界，\n"
						  "使其具有独特的外观。\n\n"
						  "Insane Shaders的一部分\n"
						  "作者: Lord of Lunacy\n\n"
						  "SHARP_CONTRAST_COMPUTE: 允许您选择在支持的API中禁用或启用\n"
						  "         计算着色器模式。\n"
						  "SHARP_CONTRAST_SIZE: 更改使用的滤波器大小。\n"
						  "         设置较低以获得更好的性能。\n"
						  "SHARP_CONTRAST_FASTER_COMPILE: 设置为0可获得轻微的性能提升，\n"
						  "         但编译时间会更长。（仅限计算模式）";
	>
	{	
		pass
