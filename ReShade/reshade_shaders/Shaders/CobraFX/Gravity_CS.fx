...
namespace COBRA_XGRV
{
    ...
    uniform float UI_GravityIntensity <
        ...
        ui_max       = 1.00;
        ui_step      = 0.01;
        ui_tooltip   = "重力强度。较高的值效果更酷但可能降低FPS！";
    ...
    uniform bool UI_UseImage <
        ui_label     = " 使用图片";
        ui_tooltip   = "将随机数生成器更改为位于Textures文件夹中的gravity_noise.png图片。\n您可以用自己的图片替换，只要名称和分辨率保持不变。";
        ui_category  = COBRA_UTL_UI_GENERAL;
    >                = true;
    ...
    uniform float UI_NoiseSize <
        ...
        ui_max       = 1.000;
        ui_step      = 0.001;
        ui_tooltip   = "噪点纹理的大小。较低的值意味着较大的噪点像素。";
        ui_category  = COBRA_UTL_UI_GENERAL;
    >                = 1.000;

    uniform bool UI_HotsamplingMode <
        ui_label     = " 热采样模式";
        ui_tooltip   = "噪点在所有分辨率下保持一致。启用此选项后调整设置，\n效果在所有分辨率下保持不变。如果不打算使用热采样，\n请关闭此选项。";
        ui_category  = COBRA_UTL_UI_GENERAL;
    >                = false;
    ...
    /*     uniform int UI_BlendMode <
            ...
            ui_type      = "radio";
            ui_spacing   = 2;
            ui_items     = "色调\0Saturate\0Desaturate\0Oversaturate\0";
    ...
    uniform bool UI_LockSaturation <
        ui_label     = " 锁定饱和度";
        ui_tooltip   = "锁定饱和度为游戏内的饱和度。";
        ui_category  = COBRA_UTL_UI_EXTRAS;
    >                = false;

    uniform bool UI_LockValue <
        ui_label     = " 锁定亮度";
        ui_tooltip   = "锁定亮度为游戏内的亮度。";
        ui_category  = COBRA_UTL_UI_EXTRAS;
    >                = false;
    ...
    uniform float3 UI_EffectTint <
        ui_label     = " 效果色调";
        ui_type      = "color";
        ui_tooltip   = "指定重力像素的色调，它们离原点越远颜色越接近此值。";
        ui_category  = COBRA_UTL_UI_EXTRAS;
    ...
    uniform float UI_BlendIntensity <
        ...
        ui_min       = 0.0;
        ui_max       = 1.0;
        ui_tooltip   = "指定应用于重力像素的混合强度。范围从0.0（无变化）\n到1.0（完全混合）。";
        ui_category  = COBRA_UTL_UI_EXTRAS;
    >                = 0.0;
    ...
    uniform int UI_BufferEnd <
        ui_type     = "radio";
        ui_spacing  = 2;
        ui_text     = " 预处理器选项：\n * GRAVITY_HEIGHT（默认值：1024）定义沿重力轴方向的效果分辨率。"
                      "该值必须为整数。较小的值提高性能但降低视觉保真度。768：性能优先 1080：高清质量。"
                      "如果希望始终以原生分辨率运行效果，请将其设置为'BUFFER_HEIGHT'。\n\n"
                      " 着色器版本：" COBRA_XGRV_VERSION;
        ui_label    = " ";
    > ;
    ...
    technique TECH_GravityCS <
        ui_label     = "重力 CS";
        ui_tooltip   = "------关于-------\n"
                       "Gravity_CS.fx让像素在游戏3D环境中向屏幕底部移动。\n"
                       "您可以按深度和颜色过滤受影响的像素。\n"
                       "它使用自定义种子（目前是Mandelbrot集合）来确定每个像素的强度。\n"
                       "请务必测试使用Textures文件夹中提供的'gravity_noise.png'图片的\n"
                       "图片随机数变体。您可以用自己的图片替换纹理，只要它是\n"
                       "1920x1080、RGBA8格式且名称相同。只使用红色强度，所以请使用红色\n"
                       "图片或灰度图片。\n"
                       "CS是Gravity.fx的计算着色器版本，在1080p以上分辨率效果最佳，\n"
                       "尽管在较低分辨率下仍可达到与Gravity.fx相似的性能。关于质量与性能\n"
                       "平衡，请阅读GRAVITY_HEIGHT预处理器的提示。\n\n"
                       "版本：    " COBRA_XGRV_VERSION "\n作者：     SirCobra\n合集： CobraFX\n"
