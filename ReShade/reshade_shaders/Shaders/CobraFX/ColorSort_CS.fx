...
namespace COBRA_XCOL
{
    ...
    uniform bool UI_ReverseSort <
        ui_label     = " 反向排序";
        ui_tooltip   = "启用时，从暗到亮排序。否则从亮到暗排序。";
        ui_category  = COBRA_UTL_UI_GENERAL;
    >                = false;
    ...
    uniform int UI_BufferEnd <
        ui_type     = "radio";
        ui_spacing  = 2;
        ui_text     = " 预处理器选项：\n * COLOR_HEIGHT（默认值：12）乘以64定义排序轴方向的效果分辨率。该值必须为整数。较小的值提高性能但降低视觉保真度。8：性能优先，12：默认，16：高清\n\n"
                      " 着色器版本：" COBRA_XCOL_VERSION;
        ui_label    = " ";
    > ;
    ...
    technique TECH_ColorSortMasking <
        ui_label     = "颜色排序：遮罩";
        ui_tooltip   = "这是着色器的遮罩部分。必须放置在颜色排序：主程序之上。\n"
                       "遮罩和主程序之间的所有效果（如黑白）只会应用于已排序的区域。\n"
                       "------关于-------\n"
                       "ColorSort_CS.fx可以沿用户指定的轴按亮度排序图像像素。\n"
                       "您可以按深度和颜色过滤受影响的像素。\n"
                       "该着色器消耗大量资源。要在质量和性能之间取得平衡，\n"
                       "请调整预处理器参数COLOR_HEIGHT。查看提示获取更多信息。\n\n"
                       "版本：    " COBRA_XCOL_VERSION "\n作者：     SirCobra\n合集： CobraFX\n"
