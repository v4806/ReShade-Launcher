...
uniform float LSPOIrrEffectSaturation < __UNIFORM_SLIDER_FLOAT1
    ...
    ui_tooltip =
        "调整遮蔽和辐照度结果的饱和度。\n"
        "\n"
        "注意这不是最终输出饱和度，而是在混合到颜色缓冲区之前应用于遮蔽和辐照度的饱和度。\n"
...
uniform float LSPOIrrOcclusionIrradianceRecovery < __UNIFORM_SLIDER_FLOAT1
    ...
    ui_tooltip =
        "调整应用于遮蔽和辐照度的恢复程度。\n"
        "\n"
        "设为0可获得整体戏剧性效果，设为1可最大程度恢复暗部和亮部区域。";
> = 0.75;
...
#define ___CATEGORY_TONING_SETTINGS___ "色调设置"
...
#define ___CATEGORY_PREPROCESSOR_DEFINITIONS_DESCRIPTIONS___ "预处理器定义说明"
