...
//	ui_max = BUFFER_WIDTH;
//	ui_step = 1;
	ui_label = "输入图像分辨率 X/Y（像素）";
	ui_tooltip = "屏幕帧缓冲中图像的分辨率（0 = 自动/全屏）";
> = int2(0, 0);

uniform int resize_method <
	ui_type = "combo";
	ui_items = " 从左上角简单缩放 \0 32:9 从中心锁定 \0 21:9 从中心锁定 \0 16:9 从中心锁定 \0 15:9 从中心锁定 \0 16:10 从中心锁定 \0 4:3 从中心锁定 \0 5:4 从中心锁定 \0 1:1 从中心锁定 \0 4:5 从中心锁定 \0 3:4 从中心锁定 \0";
	ui_label = "游戏使用的缩放方法";
	ui_tooltip = "选择游戏根据显示分辨率进行缩放的方式（简单缩放适用于大多数老游戏 / 从中心锁定比例适用于大多数新游戏）";
> = 0;

uniform bool ROTATED <
	ui_label = "旋转";
	ui_tooltip = "旋转显示（逆时针90°）";
> = false;

uniform int inp_screen_ratio <
	ui_type = "combo";
	ui_items = " 自动检测 \0 32:9 屏幕 \0 21:9 屏幕 \0 16:9 屏幕 \0 15:9 屏幕 \0 16:10 屏幕 \0 4:3 屏幕 \0 5:4 屏幕 \0 1:1 屏幕 \0 4:5 屏幕 \0 3:4 屏幕 \0";
	ui_label = "模拟屏幕比例";
> = 0;

uniform int inp_game_ratio <
	ui_type = "combo";
	ui_items = " 自动检测 \0 32:9 显示 \0 21:9 显示 \0 16:9 显示 \0 15:9 显示 \0 16:10 显示 \0 4:3 显示 \0 5:4 显示 \0 1:1 显示 \0 4:5 显示 \0 3:4 显示 \0";
	ui_label = "模拟游戏比例";
> = 0;
...
uniform float2 aspect_ratio <
	...
	ui_max = 4.0f;
	ui_step = 0.01f;
	ui_label = "手动宽高比调整 X/Y";
> = float2(1.0, 1.0);

uniform bool USE_BACKGROUND <
	ui_label = "背景";
	ui_tooltip = "在显示后面显示完整图像（全屏尺寸）";
> = false;

uniform bool USE_OFF_BEZEL <
	ui_label = "关闭边框";
	ui_tooltip = "当CRT效果未激活时显示替代边框";
> = false;
...
uniform float3 bg_col <
	ui_type = "color";
	ui_label = "背景颜色";
	ui_tooltip = "显示区域外的背景颜色（全屏尺寸）";
	ui_category = "艺术图高级设置";
> = float3(0.0, 0.0, 0.0);

uniform bool CRT_EFFECT <
	ui_label = "CRT效果";
	ui_tooltip = "启用或禁用CRT效果";
> = true;
...
//	ui_max = BUFFER_WIDTH;
//	ui_step = 1.0f;
	ui_label = "模拟纹理分辨率 X/Y（像素）";
	ui_tooltip = "用于构建扫描线效果的游戏原生分辨率（0.0 = 自动/默认）";
> = float2(0.0, 0.0);
...
uniform float2 buffer_offset <
	...
	ui_max = 5.0f;
	ui_step = 0.1f;
	ui_label = "缓冲偏移 X/Y（像素）";
	ui_tooltip = "允许设置有时需要的小偏移以完美居中于纹理";
	ui_category = "纹理高级设置";
	ui_category_closed = true;
> = float2(0.0, 0.0);
...
uniform bool CURVATURE <
	ui_label = "曲率";
	ui_tooltip = "启用或禁用屏幕曲率";
> = true;

uniform bool VERTICAL_SCANLINES <
	ui_label = "垂直扫描线";
	ui_tooltip = "无需旋转显示即可获得垂直扫描线";
> = false;

uniform int aperture_type <
	ui_type = "combo";
	ui_items = " 模拟光栅（绿/品红）\0 点阵纹理 1x1 \0 点阵纹理 2x2 \0";
	ui_label = "光栅遮罩类型";
> = 0;
...
uniform bool OVERSAMPLE <
	ui_label = "过采样";
	ui_tooltip = "启用光束轮廓3倍过采样（改善扫描线+曲率引起的摩尔纹效果）";
	ui_category = "CRT高级设置";
> = true;
...
uniform float ovs_boost <
	...
	ui_step = 0.01f;
	ui_label = "过采样增强";
	ui_tooltip = "尝试进一步减少摩尔纹效果，但会破坏像素外观";
	ui_category = "CRT高级设置";
> = 1.0f;
...
uniform float BloomBlurOffset <
	...
	ui_step = 0.01f;
	ui_label = "泛光模糊偏移";
	ui_tooltip = "模糊半径的额外调整。小于1.00的值会减小半径。";
	ui_category = "泛光高级设置";
	ui_category_closed = true;
> = 1.6f;
...
uniform bool USE_BEZEL <
	ui_label = "边框";
	ui_tooltip = "添加全局边框/叠加层（全屏尺寸）";
> = false;

uniform bool USE_FRAME <
	ui_label = "屏幕边框";
	ui_tooltip = "在显示上添加屏幕边框叠加层（显示尺寸）";
> = false;

uniform bool USE_OVERLAY <
	ui_label = "屏幕叠加";
	ui_tooltip = "在显示区域上添加屏幕叠加层（显示尺寸）";
> = false;
...
uniform bool ACTIVATION_PIXEL_TEST <
	ui_label = "启用像素测试";
	ui_tooltip = "仅当测试的2个像素颜色匹配时才启用效果（位置应在1920x1080分辨率的后缓冲中定义）";
	ui_category = "像素测试高级设置";
	ui_category_closed = true;
> = false;
