...
uniform int MXAO_GLOBAL_SAMPLE_QUALITY_PRESET <
	ui_type = "combo";
    ui_label = "采样质量";
	ui_items = "低\0中\0高\0非常高\0极致\0极端\0无视性能\0";
	ui_tooltip = "全局质量控制，主要性能调节参数。较大的半径可能需要更高质量。";
    ui_category = "全局设置";
> = 1;
...
uniform int SHADING_RATE <
	ui_type = "combo";
    ui_label = "着色率";
	ui_items = "全速率\0半速率\0四分之一速率\0";
	ui_tooltip = "0: 每帧渲染所有像素\n1: 每帧只渲染50%的像素\n2: 每帧只渲染25%的像素";
    ui_category = "全局设置";
> = 1;
...
uniform float MXAO_SAMPLE_RADIUS <
    ...
    ui_label = "采样半径";
	ui_tooltip = "MXAO的采样半径，值越大意味着更大范围的遮挡效果，但会减少细节表现。";  
    ui_category = "全局设置";      
> = 2.5;
...
uniform float MXAO_SSAO_AMOUNT <
    ...
    ui_label = "环境光遮蔽强度";        
	ui_tooltip = "AO效果的强度。设置过高可能导致黑色剪裁。";
    ui_category = "混合设置";
> = 0.8;
...
uniform float MXAO_FADE_DEPTH <
    ...
    ui_label = "淡出距离";
	ui_min = 0.0; ui_max = 1.0;
	ui_tooltip = "MXAO的淡出距离。值越高在更远的区域显示MXAO效果。";
    ui_category = "混合设置";
> = 0.25;
...
ui_type = "radio";
    ...
    ui_text = 
            ...
            ":\n\n0: 真实环境光遮蔽（高对比度，快速）\n"
                 "1: 立体角（更平滑，最快）\n"
                 "2: 可见性位掩码（仅DX11+，最高质量，较慢）\n"
                 "3: 带立体角的可见性位掩码（类似2，但更平滑）\n"
            "\n"
            TOKENIZE(MXAO_USE_LAUNCHPAD_NORMALS)
            ":\n\n0: 实时计算法线向量（快速）\n"
                 "1: 使用iMMERSE Launchpad的法线（较慢）\n"
                 "   允许使用Launchpad的平滑法线功能。";
>;
...
<
    ...
    ui_tooltip =        
        ...
        "MXAO是一种高质量、高性能的屏幕空间环境光遮蔽（SSAO）效果，能够准确模拟暗角\n"
        "和缝隙中的漫反射阴影。\n"
        "\n"
        "\n"
