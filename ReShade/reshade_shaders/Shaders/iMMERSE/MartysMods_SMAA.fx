...
uniform int EDGE_DETECTION_MODE < 
    ui_type = "combo";
	ui_items = "亮度边缘检测\0颜色边缘检测(最大值)\0颜色边缘检测(加权)\0深度边缘检测\0";
	ui_label = "边缘检测类型";
> = 1;
...
uniform float SMAA_THRESHOLD < 
	...
	ui_tooltip = "边缘检测阈值。如果SMAA遗漏了一些边缘，请尝试稍微降低此值。";
	ui_label = "边缘检测阈值";
> = 0.10;
...
uniform float SMAA_DEPTH_THRESHOLD < 
	...
	ui_tooltip = "深度边缘检测阈值。如果SMAA遗漏了一些边缘，请尝试稍微降低此值。";
	ui_label = "深度边缘检测阈值";
> = 0.01;
...
uniform int SMAA_MAX_SEARCH_STEPS_DIAG < 
	...
	ui_min = 0; ui_max = 25;
	ui_label = "对角线最大搜索步数";
	ui_tooltip = "确定SMAA将搜索对角线锯齿边缘的半径";
> = 16;
...
uniform float SMAA_PREDICATION_STRENGTH < 
    ui_type = "slider";
	ui_min = 0; ui_max = 4;
	ui_tooltip = "局部降低阈值的强度。";
	ui_label = "预测强度";
> = 0.4;

uniform int DebugOutput < 
    ui_type = "combo";
	ui_items = "无\0显示边缘\0显示权重\0";
	ui_label = "调试输出";
> = false;
...
<
    ui_label = "iMMERSE: SMAA抗锯齿";
    ui_tooltip =        
        "                               MartysMods - SMAA                              \n"
         "                   MartysMods Epic ReShade Effects (iMMERSE)                 \n"
        "______________________________________________________________________________\n"
        "\n"
        "此版本的'增强子像素形态抗锯齿'(SMAA)实现根据设置可提供比原始版本高达两倍的性能。\n"
        "\n"
        "\n"
