...
===============================================================================
    ...
    Launchpad 是一个预处理效果，用于准备各种数据以供后续着色器使用。

    作者:         Pascal Gilcher

    更多信息:      https://martysmods.com
...
uniform int OPTICAL_FLOW_Q <
    ...
    ui_label = "光流质量";
	ui_items = "低\0中\0高\0";
	ui_tooltip = "更高的设置会产生更准确的结果，但会降低性能。";
	ui_category = "运动估计/光流";
> = 0;
...
uniform int OPTICAL_FLOW_OPT <
	ui_type = "combo";
    ui_label = "光流优化器";
	ui_items = "Sophia（二阶Hessian优化器）\0牛顿法\0";
	ui_tooltip = "Launchpad的光流使用梯度下降法，类似于AI训练。\n\n"
				 "Sophia在高品质设置下收敛更好。\n"
				 "牛顿法在低设置下下降更快但可能收敛较差。";
	ui_category = "运动估计/光流";
> = 0;
...
uniform bool ENABLE_SMOOTH_NORMALS <	
	ui_label = "启用平滑法线";
	ui_tooltip = "过滤法线缓冲区以减少MXAO和RTGI中的低多边形外观。"
	"\n\n"
	"光照算法依赖于法线向量，法线描述了场景中几何体的方向。由于ReShade无法访问游戏本身的法线，\n"
	"它们是从深度缓冲区生成的。然而，这个过程是有损的，并且不包含法线贴图和平滑组。\n"
	"因此，它们代表了真实的（块状）物体形状，使用它们计算的光照会使几何体的低多边形外观变得明显。\n";
	ui_category = "法线贴图";	
> = false;

uniform bool ENABLE_TEXTURED_NORMALS <	
	ui_label = "启用纹理法线";
	ui_tooltip = "基于颜色信息估算表面浮雕，以获得更准确的几何表示。\n"
	             "需要先启用平滑法线！";	
	ui_category = "法线贴图";	
> = false;
...
uniform int DEBUG_MODE < 
    ui_type = "combo";
	ui_items = "全部\0光流\0光流矢量\0法线\0深度\0";
	ui_label = "调试输出";
	ui_category = "调试";
> = 0;
#endif
