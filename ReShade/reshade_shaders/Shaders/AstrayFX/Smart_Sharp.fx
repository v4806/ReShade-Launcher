...
uniform bool Depth_Map_Flip <
	ui_label = "深度图翻转";
	ui_tooltip = "如果深度图上下颠倒，请翻转它。";
	ui_category = "深度缓冲";
> = DB_X;

uniform bool No_Depth_Map <
	ui_label = "无深度图";
	ui_tooltip = "如果您没有深度缓冲，请开启此选项。";
	ui_category = "深度缓冲";
> = false;
...
uniform float Sharpness <
    ...
    ui_tooltip = "通过调整此滑块从零到一来增加图像的锐度。\n"
				 "零 = 不锐化，一 = 完全锐化，超过一 = 更加锐利。\n"
				 "0.625 等同于 AMD FidelityFX 对比度自适应锐化的最大设置。\n"
				 "默认值为 0.625。";
	ui_category = "双边CAS";
> = 0.625;
...
uniform float B_Grounding <
    ...
    ui_label = "粗糙度";
    ui_min = 0.0; ui_max = 1.0;
	ui_tooltip = "像咖啡一样选择你想要这个着色器有多粗糙，实际上只是调整滤镜的半径。\n"
				 "细腻 -> 中等 -> 粗糙，选择你认为好看的效果。\n"
				 "默认值为 0.0。";
	ui_category = "双边CAS";
> = 0.0;
...
uniform bool CAM_IOB <
	ui_label = "CAM忽略过亮";
	ui_tooltip = "不在遮罩中允许过亮区域，而是允许对该区域进行锐化。\n"
				 "我认为保持此选项开启更加准确。";
	ui_category = "双边CAS";
> = false;
...
uniform bool CA_Mask_Boost <
	ui_label = "CAM增强";
	ui_tooltip = "这会增强着色器的对比度自适应遮罩部分的效果。";
	ui_category = "双边CAS";
> = false;

uniform bool CA_Removal <
	ui_label = "CAM移除";
	ui_tooltip = "这会移除着色器额外的对比度自适应遮罩部分。\n"
				 "请记住，此滤镜已经具有一定程度的对比度遮罩。\n"
				 "这是为喜欢双边锐化原始外观的人准备的。\n"
				 "双边滤镜的降噪功能会自动应用。";
	ui_category = "双边CAS";
> = false;
...
uniform int Local_Motion <
	ui_type = "combo";
	ui_items = "通用运动\0局部运动\0";
	ui_label = "视图模式";
	ui_tooltip = "用于选择通用运动和局部运动。\n"
				 "默认为通用运动。";
	ui_category = "运动双边CAS";
> = 0;
...
uniform float MDSM <
    ...
    ui_tooltip = "运动检测锐化倍增器。\n"
				 "这是用户设置的倍增器，用于控制增加基础锐化的程度。\n"
				 "在基础锐度下，5倍应该足够。请尝试调整此值。\n"
				 "锐化倍增器为2意味着是用户设置锐化强度的两倍。\n"
				 "默认值为1。";
	ui_category = "运动双边CAS";
> = 1.0;
#else
...
uniform int Debug_View <
	ui_type = "combo";
	ui_items = "正常\0锐化视图\0深度遮罩\0CAS遮罩\0噪点遮罩\0";
	ui_label = "调试视图";
	ui_tooltip = "用于调试智能锐化并让您微调着色器。";
	ui_category = "调试";
> = 0;

uniform int F_DeNoise <
	ui_type = "combo";
	ui_items = "关\0一级降噪\0二级降噪\0";
	ui_label = "智能降噪选项";
	ui_tooltip = "此降噪器可以帮助处理图像中的噪点。\n"
				 "一级降噪仅与智能锐化配合使用。\n"
				 "二级降噪可与其他着色器配合使用，但效果稍强。\n"
				 "三级降噪暂不可用，将在未来添加。";
	ui_category = "额外菜单";
> = false;
...
< ui_tooltip = "建议：你可以在ReShade主界面右下角启用'性能模式复选框'。\n"
			   "             当然，请在设置好Smart Sharp参数后再这样做。"; >
{
