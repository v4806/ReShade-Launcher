...
uniform bool EnableLevels <
	ui_tooltip = "启用或禁用电视<>电脑或自定义色彩范围的色阶调整";
> = true;

uniform float3 InputBlackPoint < __UNIFORM_COLOR_FLOAT3
	ui_tooltip = "黑点是新的黑色 - 字面意思。\n比此值更暗的所有内容都将变成完全黑色。";
...
uniform float3 ColorRangeShift < __UNIFORM_COLOR_FLOAT3
	ui_tooltip = "某些游戏（如看门狗2）的色彩范围16-235下移到了0-219，\n因此添加了此选项以在扩展色彩范围之前上移它。\n此处输入的RGB值将直接添加到默认颜色值。\n目前在游戏中无法使用负值，但可以在着色器中添加（如需下移）。\n0表示禁用偏移。";
> = float3(0/255.0f, 0/255.0f, 0/255.0f);

uniform int ColorRangeShiftSwitch < __UNIFORM_SLIDER_INT1
	ui_min = -1; ui_max = 1;
	ui_tooltip = "解决Reshade UI中缺少负颜色值的问题：\n-1 下移，\n1 上移，\n0 禁用";
> = 0;
...
uniform bool HighlightClipping <
	ui_tooltip = "两点之间的颜色将被拉伸，这会增加对比度，但超出这两点的细节会丢失（这称为裁切）。\n高亮显示裁切的像素。\n红色 = 高光部分丢失了一些细节，\n黄色 = 高光部分丢失了所有细节，\n蓝色 = 阴影部分丢失了一些细节，\n青色 = 阴影部分丢失了所有细节。";
> = false;
...
uniform bool enableACESFilmRec2020old <
	ui_tooltip = "启用或禁用旧版ACES以改善对比度和亮度";
> = false;
