...
uniform float samples <
	...
	ui_label = "采样数";
	ui_tooltip = "GI采样数量用于增加采样数，副作用是可以减少噪点。";
	ui_category = "全局光照";
> = 6;
...
uniform float Target_Lighting <
	...
	ui_label = "定向光照";
	ui_tooltip = "让你专门针对直接光照和/或间接光照，以便着色器使用你的调整进行GI计算。\n"
			     "负值针对间接光照，正值针对直接光照。\n"
				 "默认值为[0.05]，零表示全图采样。";
	ui_category = "全局光照";
> = 0.05;
...
uniform float2 NCD <
	...
	ui_label = "近处细节";
	ui_tooltip = "让你调整相机附近物体的细节，如武器手部GI。\n"
			     "第二个选项用于超出范围的游戏内武器手部。\n"
...
uniform float PCGI_Fade < //Blame the pineapple for this option.
	...
	ui_label = "深度淡出";
	ui_tooltip = "基于深度缩放的GI应用强度，用于控制淡入淡出。\n"
			     "可设置为0到1，设为零表示无剔除。\n"
			     "默认值为0.0。";
	ui_category = "全局光照";
> = 1.0;
...
uniform float2 Reflectivness <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "漫反射量";
	ui_tooltip = "基本控制地面上的光照漫反射外观。\n"
			     "默认值为[1.0 | 0.5]。1是最大漫反射，值为长度。";
	ui_category = "全局光照";
> = float2(1.0,0.5);

uniform bool Scattering<
	ui_label = "次表面光传输";
	ui_tooltip = "启用次表面光传输功能以创建次表面散射效果。\n"
			     "默认为关闭。";
	ui_category = "次表面散射";
> = false;
...
uniform float Wrap <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "上层散射";
	ui_tooltip = "控制光照包裹形状因子以调整次表面散射效果。\n"
			     "默认值为[0.5]。";
	ui_category = "次表面散射";
> = 0.5;
/*
...
uniform float Deep_Scattering <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "深层散射";
	ui_tooltip = "控制厚度估算形状因子以创建深层组织散射效果。\n"
			     "默认值为[0.1]。";
	ui_category = "次表面散射";
> = 0.1;
...
uniform float Luma_Map <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "组织亮度图";
	ui_tooltip = "控制亮度图，让明亮的光线近似深层组织穿透。\n"
			     "默认值为[0.5]。";
	ui_category = "次表面散射";
> = 0.5;
uniform float3 Internals < // We are all pink and fleshy on the inside?
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "内部肉色";
	ui_tooltip = "由于无法知道深层组织的内部颜色，你需要自己设置RGB值。\n"
			     "默认值为[R 0.25] [B 0.0] [G 0.0] [L 0.5]。";
	ui_category = "次表面散射";
...
uniform float2 Diffusion_Saturation_Power <
	ui_type = "slider";
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "次表面模糊与肉色饱和度";
	ui_tooltip = "漫反射模糊用于柔化光照，通过模拟皮肤对光线的影响使人物更加逼真。\n"
			     "模拟光线漫反射，红色通道模糊多于其他颜色。\n"
			     "默认值为[0.5] [0.5]。";
	ui_category = "次表面散射";
> = float2(0.25,0.5);
...
uniform float SSS_Seek <
	...
	ui_label = "皮肤检测距离";
#endif
	ui_tooltip = "让你控制人类肤色检测算法在SSLT中需要搜索的距离。\n"
#if LutSD
			     "默认值为[0.25]和[0.5]。";
#else
				 "第二个选项仅在使用特殊LUT进行自定义或更精确的肤色检测时显示。\n"
				 "你可以在此获取LUT @ https://github.com/BlueSkyDefender/AstrayFX/wiki/Subsurface-Light-Transport\n"
			     "默认值为[0.25]。";
#endif
	ui_category = "次表面散射";
#if LutSD
...
uniform int Sky_Emissive_Selection < //Emissive_Mode //Sky_Contribution
	ui_type = "combo";
    ui_label = "方向性天空与自发光模式";
    ui_tooltip = "用于添加方向性天空和/或自发光模式。\n"
    			"\n"
				"方向性天空颜色：\n"
				"允许使用天空颜色贡献直接影响世界。\n"
    			"\n"
				"自发光模式：\n"
				"基于发光体的近似信息，使所有明亮物体成为'自发光物体'。\n"
				"\n"
    			"默认为关闭。";
    ui_items = "关\0方向性天空颜色\0自发光模式\0天空与自发光模式\0";
	ui_category = "补充贡献";
    > = 0;
...
uniform float D_Irradiance <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "远距辐照度";
	ui_tooltip = "远距辐照度让你控制来自远处地面折射光的辐照度。\n"
			     "默认值为[0.0]，零表示关闭。";
	ui_category = "补充贡献";
> = 0.0;
...
uniform float2 PCGI_2DTexture_Detail <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "纹理细节与衰减";
	ui_tooltip = "让你为PCGI添加纹理细节，使2D纹理信息应用于GI。\n"
				 "通过调整第一个滑块开启，然后用第二个滑块调整衰减。\n"
			     "默认值为[0.0,0.5]，零表示关闭。";
	ui_category = "补充贡献";
> = float2(0.0,1.0);
#else
...
uniform float Trim <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "修剪";
	ui_tooltip = "通过限制GI影响周围物体的距离来修剪GI。\n"
				 "方向性天空颜色会受此选项的负面影响。\n"
				 "主要用于自发光模式。\n"
			     "默认值为[0.0]，零表示关闭。";
	ui_category = "补充贡献";
...
uniform float HDR_BP <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "HDR提取强度";
	ui_tooltip = "用于调整HDR强度，你可以覆盖此值设为1.5或其他值。\n"
				 "默认值为[0.5]，零表示关闭。";
	ui_category = "图像";
> = 0.5;
...
uniform float Offset <
	...
	ui_min = -1.0; ui_max = 1.0;
	ui_label = "深度图偏移";
	ui_tooltip = "深度图偏移用于不符合规范的ZBuffer。\n"
				 "你很少需要使用此选项。\n"
				 "用于对DM 0或DM 1进行调整。\n"
				 "默认值为零，表示关闭。";
	ui_category = "深度图";
...
uniform int Debug <
	ui_type = "combo";
	ui_items = "RadiantGI\0辐照度图\0光源图\0深度与法线\0";
	ui_label = "调试视图";
	ui_tooltip = "查看调试缓冲区。";
	ui_category = "额外选项";
> = 0;
...
uniform int SamplesXY <
	...
	ui_min = 0; ui_max = 20;
	ui_label = "降噪强度";
	ui_tooltip = "提高或降低最终降噪器使用的采样数，进而影响性能。\n"
				 "副作用是使图像更平滑，获得类似法线的平滑效果。\n"
				 "默认值为7，可以稍微覆盖。";
	ui_category = "额外选项";
> = 6;
...
uniform float GI_Res <
	...
	ui_label = "分辨率";
	ui_tooltip = "GI分辨率用于以牺牲质量为代价提高性能。\n"
				 "与默认值偏差越大，噪点越少，反之亦然。\n"
				 "默认值根据当前分辨率自动调整。";
	ui_category = "额外选项";
> = RSRes;
...
> = DC_W;
	...
	uniform bool BD_Options <
		ui_label = "切换桶形畸变";
		ui_tooltip = "如果你修改了游戏以移除桶形畸变，请使用此选项。";
	ui_category = "深度校正";
	> = !true;
	#else
...
ui_tooltip = "RadiantGI: 点云全局光照。"; >
#else
technique PCGI_One
< togglectrl = true; toggle = 0x2E; ui_label = "PCGI¹";
ui_tooltip = "Alpha: 全局光照主生成器。¹"; >
#endif
{
		pass PastFrame
	{
