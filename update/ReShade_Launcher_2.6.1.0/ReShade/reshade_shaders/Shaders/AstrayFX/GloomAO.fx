...
uniform int SSDO_MipSampling <
	ui_type = "combo";
	ui_items = "全分辨率\0半分辨率\0四分之一分辨率\0全分辨率自适应\0半分辨率自适应\0四分之一分辨率自适应\0";
    ui_label = "采样质量";
    ui_tooltip = "使用此项通过采样Mipmaps来提高性能。\n"
				 "较低质量设置下伪影更明显。";
	ui_category = "SSDO";
> = 4;
...
uniform int SSDO_Levels <
    ...
    ui_min = 0; ui_max = 64;
    ui_label = "采样数";
    ui_tooltip = "采样数滑块用于增加或减少采样数量，副作用是以性能为代价减少噪点。";
	ui_category = "SSDO";
> = 32;
...
uniform float SSDO_SampleRadius <
    ...
    ui_label = "采样半径";
    ui_tooltip = "此项允许你扩展SSDO算法的采样半径或采样范围。\n"
				 "设置过高会降低性能。";//High values reduce cache coherence, This will lead to cache misses and decrease performance.
	ui_category = "SSDO";
> = 2500.0;
...
uniform float2 Set_2DTexture_Detail <
	...
	ui_label = "纹理细节与衰减";
	ui_tooltip = "让你为PCGI添加纹理细节，以便将2D纹理信息应用到GI。\n"
				 "通过调整第一个滑块打开此功能，然后用第二个调整衰减。\n"
			     "默认值为[0.0,0.5]，零为关闭";
	ui_category = "补充贡献";
> = float2(0.0,1.0);
...
uniform float2 NCD <
	...
	ui_min = 0.0; ui_max = 1.0;
	ui_label = "近处细节";
	ui_tooltip = "让你调整靠近相机的物体细节和/或武器手部AO。\n"
			     "第二个选项用于超出范围的游戏内武器手部。\n"
			     "默认值为[近处细节 X 0.125] [武器手部 Y 0.0]";
	ui_category = "SSDO";
> = float2(0.125,0.0);
...
uniform float SSDO_Trimming <
    ...
    ui_min = 0; ui_max = 0.5;
    ui_label = "深度修剪";
    ui_tooltip = "使用此项限制图像中AO的局部衰减，也称为深度范围检查。";
	ui_category = "SSDO";
> = 0.1;
...
uniform float SSDO_Fade <
	...
	ui_label = "深度淡出";
	ui_tooltip = "基于深度缩放的SSDO应用强度，用于控制淡入淡出。\n"
			     "可以设置从-1到1，设置为零表示无剔除。\n"
			     "默认值为0.5。";
	ui_category = "SSDO";
> = 1.0;
...
uniform int BM <
    ...
    ui_items = "混合\0叠加\0Softlight\0Add\0";
    ui_category = "Image";
    > = 0;
*/
...
uniform int SSDO_X2 < //Thank you Nathaniel for the name
	ui_type = "combo";
	ui_items = "关\0平方输入\0亮度正向平方\0亮度负向平方\0";
	ui_label = "平方输入";
	ui_tooltip = "此选项对SSDO的后缓冲输入进行平方处理。\n"
				 "你也可以偏向明亮区域或暗部区域。\n"
				 "这基本上影响GI颜色和AO，默认为关闭。";
	ui_category = "图像";
> = 0;
...
uniform float SSDO_Intensity_Masking <
    ...
    ui_min = 0.0; ui_max = 1.0;
    ui_label = "强度遮罩";
    ui_tooltip = "遮罩强光源，使其不受AO影响。";
	ui_category = "图像";
> = 0.0;

uniform int Depth_Map <
	ui_type = "combo";
	ui_items = "DM0 正常\0DM1 反转\0";
	ui_label = "深度图选择";
	ui_tooltip = "深度缓冲的线性化，也称为深度图。\n"
			     "DM0是Z-正常，DM1是Z-反转。\n";
	ui_category = "深度图";
> = DA_W;
...
uniform float SSDO_Max_Depth <
	...
	ui_label = "最大深度";
	ui_tooltip = "SSDO最大深度用于限制从深度缓冲获取信息的最大范围。\n"
			     "可以把它想象成一堵墙，所有信息在此停止，请注意这与深度淡出不同。\n"
			     "默认值为0.999。";
	ui_category = "深度图";
> = 0.999;
...
uniform int SamplesXY <
	...
	ui_min = 1; ui_max = 6;
	ui_label = "降噪强度";
	ui_tooltip = "这会提高或降低最终降噪器使用的采样数，从而影响性能。\n"
				 "这也有平滑图像的副作用，可以获得类似法线的平滑效果。\n"
				 "默认值为3，你可以稍微覆盖此设置。";
	ui_category = "额外选项";
> = 3;
...
uniform float Persistence <
	...
	ui_label = "TAA强度";
	ui_tooltip = "增加帧的持久性，这是时间抗锯齿的核心部分。\n"
				 "默认值为0.125，值为1.0表示关闭。";
	ui_category = "额外选项";
> = 0.125;
...
uniform bool Dither_SSDO <
	ui_label = "抖动SSDO";
	ui_tooltip = "为AO添加噪点，以便在某些游戏中减少色带。";
	ui_category = "额外选项";
> = true;
...
uniform float2 Image_Position_Adjust<
	...
	ui_min = -1.0; ui_max = 1.0;
	ui_label = "水平与垂直位置";
	ui_tooltip = "如果图像位置偏移，请调整。默认值为零。";
	ui_category = "深度校正";
> = float2(DD_Z,DD_W);

uniform int Easy_SS_Scaling <
	ui_type = "combo";
	ui_items = "关\0DLSS质量\0DLSS平衡\0DLSS性能\0DLSS超级性能\0";
	ui_label = "超级采样缩放";
	ui_tooltip = "使用此项调整深度以匹配主屏幕。\n"
				 "默认为开启。";
	ui_category = "深度校正";
> = 0;
#else
...
> = DC_W;
	...
	uniform bool BD_Options <
		ui_label = "切换桶形畸变";
		ui_tooltip = "如果你修改了游戏以移除桶形畸变，请使用此项。";
	ui_category = "深度校正";
	> = !true;
	#else
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static const float  CH_D    = float(0x79997), CH_E    = float(0xf171f), CH_F    = float(0xf1711),
					CH_H    = float(0x99f99), CH_I    = float(0xf444f), CH_N    = float(0x9bd99), 
					CH_O    = float(0x69996), CH_P    = float(0x79971), CH_T    = float(0xf4444), 
					CH_3    = float(0x69496), CH_FSTP = float(0x00002);
...
}   float DT_Information(){ float DT_Text = T_01(); return clock <= DT_Text; }
...
float4 Out(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	float Gradient = (1-texcoord.y*50.0+48.85)*texcoord.y-0.500, BT = smoothstep(0,1,sin(clock*(3.75/1000))), Size = 1.1, Depth3D;
	float4 Color = SSDOMixing(texcoord);

	[branch] if(Text_Info_Plus())
	{ // Set a general character size...
		...
		//Website
		return Depth3D ? Minimize_Web_Info ? lerp(Gradient + Depth3D,Color,saturate(Depth3D*0.98)) : Gradient : Color;
	}
	else
